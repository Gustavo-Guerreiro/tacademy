...
<main id="mainPostagemCompleta">
  ...
  <div class="card">
    <div class="card-header">
      Por: <% out.print(rs.getString(2)); out.print(rs.getInt(6)); %>
    </div>
    <div class="card-body">
      <h5 class="card-title"> <% out.print(rs.getString(3)); %> </h5>
      <p class="card-text"> <% out.print(rs.getString(4)); %> </p>
        <%
        if (session.getAttribute("tipo") != null && session.getAttribute("idUsuario") != null) {
            if (session.getAttribute("tipo").equals("A") || Integer.parseInt(session.getAttribute("idUsuario").toString() ) == rs.getInt(6)) {
        %>
